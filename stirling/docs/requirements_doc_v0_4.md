...
| ID | Requirement | Target | Status |
| --- | --- | --- | --- |
| NFR-34 | Classical controller reform time (MATLAB smoke test) | Reform after disturbance kick ≤2.0 s | CONFIRMED — 0.60 s achieved |
| NFR-35 | Classical controller minimum inter-drone separation | ≥d_sep_min (FR-15 TBC, currently 0.40 m floor) | CONFIRMED — 0.54 m in smoke test |
| NFR-36 | Classical controller C port | Logic ported to dronelib.h / velocity_controller.h inside PufferLib env c_step before Stage 3 | PENDING — MATLAB draft complete |
| NFR-37 | Safety filter wraps controller + residual | APF/CBF filter applied to u_classic + k_res×Δv every cycle; hard separation guarantee regardless of RL residual | Confirmed (APF); CBF optional upgrade |
...
Classical controller — MATLAB draft complete; smoke-tested (reform 0.60 s, min sep 0.54 m). Port to C pending (NFR-36).
